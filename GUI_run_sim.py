...
USE_SIM = True  # ← flip to False to use real hardware controllers

if USE_SIM:
    # Map real controllers to mocks for simulation
    from mock_controllers import MockLF50Controller as LoadFrameController
    class LF50Mover:
        def __init__(self, *args, **kwargs):
            pass
    from mock_controllers import MockSTDDPCController as STTDPCController
    from mock_controllers import MockSerialPad as SerialPadReader
else:
...
class HomePage(QWidget):
    def __init__(self):
        ...
        layout.setAlignment(Qt.AlignCenter)
          # match real polling interval if needed
...
class MainWindow(QMainWindow):
    ...
    def _list_devices(self):
        """Return available SerialPad devices. In simulation, return a mock list."""
        if USE_SIM or self.serial_pad is None or not hasattr(self.serial_pad, 'list_devices'):
            return ["SerialPad-Sim (MOCK)"]
        try:
            return self.serial_pad.list_devices()
        except Exception:
            # Fall back gracefully if hardware query fails
            return []
    ...
    def handle_live_reading(self, reading):
        ...
        if hasattr(self, "view_page") and self.view_page:
    ...
    def start_test(self, _):
        ...
        # Autofill current pressures without prompting (prompt only if not sim and still unknown)
        def _read_pressure_safe(ctrl):
            try:
                return float(ctrl.read_pressure())
            except Exception:
                return None
        ...
        for stage_data in self.setup_page.stage_data_list:
            if stage_data.stage_type in ("Saturation", "Shear"):
                # Cell
                need_cell = not hasattr(stage_data, "current_cell_pressure") or stage_data.current_cell_pressure is None
                if need_cell:
                    v = _read_pressure_safe(self.cell_pressure_controller)
                    if v is not None:
                        stage_data.current_cell_pressure = v
                    elif USE_SIM:
                        stage_data.current_cell_pressure = 0.0  # no dialog in sim
                    else:
                        val, ok = QInputDialog.getDouble(None, "Current Cell Pressure",
                                                         "Enter current cell pressure (kPa):", 0, 0, 1000, 1)
                        if not ok:
                ...
                # Back
                need_back = not hasattr(stage_data, "current_back_pressure") or stage_data.current_back_pressure is None
                if need_back:
                    v = _read_pressure_safe(self.back_pressure_controller)
                    if v is not None:
                        stage_data.current_back_pressure = v
                    elif USE_SIM:
                        stage_data.current_back_pressure = 0.0  # no dialog in sim
                    else:
                        val, ok = QInputDialog.getDouble(None, "Current Back Pressure",
                                                         "Enter current back pressure (kPa):", 0, 0, 1000, 1)
                        if not ok:
                            self.log("[✗] Test cancelled by user.")
                            return
        ...
        if (not USE_SIM) and not (self.lf_controller and self.cell_pressure_controller and self.back_pressure_controller and self.serial_pad):
            self.log("[✗] All devices must be connected before starting a test.")
            return

        test_config = self.setup_page.get_test_config()

        self.test_manager = TriaxialTestManager(
            self.lf_controller,
            self.cell_pressure_controller,
            self.back_pressure_controller,
            self.serial_pad,
            test_config,
            main_window=self,  # ✅ give it the MainWindow reference
            log=self.log
        )
        ...
        self.test_manager.test_started.connect(self._emit_test_anchor)
        self.test_manager.stage_started.connect(self._emit_stage_anchor)
    ...
    def advance_to_next_stage(self):
        ...
        # Stop current stage first
        self.test_manager.stop_stage()

        # Prompt to save graphs
        resp = QMessageBox.question(
            self,
            "Save Graphs",
            "Do you want to save stage graphs before continuing?",
            QMessageBox.Yes | QMessageBox.No
        )
        if resp == QMessageBox.Yes:
            self.view_page.export_graphs()

        # Clear only stage-start graphs
        self.view_page.clear_stage_start_graphs()
    ...
    def _emit_test_anchor(self, ts: float):
        self._emit_anchor_sample(ts)

    def _emit_stage_anchor(self, ts: float):
        self._emit_anchor_sample(ts)

    def _emit_anchor_sample(self, ts: float):
        def read_safe(fn, default=0.0):
            try:
                return float(fn())
            except Exception:
                return default
        ...
        anchor = {
            "timestamp": ts,
            "cell_pressure_kpa": read_safe(getattr(self.cell_pressure_controller, "read_pressure", lambda: 0.0)),
            "back_pressure_kpa": read_safe(getattr(self.back_pressure_controller, "read_pressure", lambda: 0.0)),
            "cell_volume_mm3":  read_safe(getattr(self.cell_pressure_controller, "read_volume", lambda: 0.0)),
            "back_volume_mm3":  read_safe(getattr(self.back_pressure_controller, "read_volume", lambda: 0.0)),
            "position_mm":      read_safe(getattr(self.lf_controller, "read_position", lambda: 0.0)),
            "serial_channels":  read_safe(getattr(self.serial_pad, "read_channels", lambda: []), default=[]),
        }
        # Reuse your normal path so derived times get computed the same way
        self.handle_live_reading(anchor)     # pushes a point at exactly t=0 to the graphs


    def _init_sim(self):
        # Create mock devices
        try:
            self.lf_controller = LoadFrameController()
        except Exception:
            self.lf_controller = None

        try:
            # pass a logger if your mock accepts it; ignore if it doesn't
        ...
        # Reflect connection state in Manual page buttons
        self.manual_page.set_axial_enabled(self._is_connected(self.lf_controller))
        self.manual_page.set_cell_enabled(self._is_connected(self.cell_pressure_controller))
        self.manual_page.set_back_enabled(self._is_connected(self.back_pressure_controller))

        # Push one initial reading so Data/Test views have data immediately
        def read_safe(fn, default=0.0):
            try:
                return fn()
            except Exception:
        ...
        self.handle_live_reading(reading)

        # Optional: keep sim ticking every 500 ms when not in a test
        ...
        def pump():
            ...
            self.handle_live_reading(r)
        self._sim_timer.timeout.connect(pump)
        self._sim_timer.start()


    def _sim_pump(self):
        # deprecated: test manager drives plots; keep no-op for safety
        return
    ...
    def handle_live_reading(self, reading):
        ...
        # Use test/stage clocks from the manager when available
        tm = getattr(self, "test_manager", None)
        test0 = getattr(tm, "test_start_ts", None)
        stage0 = getattr(tm, "stage_start_ts", None)

        ts = reading.get("timestamp")
        if isinstance(ts, (int, float)):
            reading["time_since_test_start_s"]  = ts - (test0  if test0  is not None else ts)
            reading["time_since_stage_start_s"] = ts - (stage0 if stage0 is not None else ts)

        # Push to the view once
        if hasattr(self, "view_page") and self.view_page:
            self.view_page.shared_data = reading
            self.view_page.update_plot(reading)
