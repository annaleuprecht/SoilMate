...
class STTDPCController:
    ...
    def ramp_pressure(self, target_kpa, rate_kpa_per_min):
        current = read_current_pressure()
        step = 0.5  # kPa per step
        interval = (step / rate_kpa_per_min) * 60  # seconds

        while abs(current - target_kpa) > step:
            next_p = current + step if target_kpa > current else current - step
            send_pressure_command(next_p)
            time.sleep(interval)
            current = read_current_pressure()
