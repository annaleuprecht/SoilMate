...
class LoadFrameController:
    ...
    def send_velocity(self, velocity):
        ...
        if velocity < self.MIN_VELOCITY or velocity > self.MAX_VELOCITY:
            self.log(f"[✗] Velocity {velocity} mm/min out of range ({self.MIN_VELOCITY} to {self.MAX_VELOCITY}).")
            return

        try:
            mover = LF50Mover(self.dev, log=self.log)
            mover.send_velocity(velocity)
            self.log(f"[→] Sent velocity command: {velocity:.2f} mm/min")
        except Exception as e:
            self.log(f"[✗] Failed to send velocity: {e}")
